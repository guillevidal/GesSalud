...
const { Agenda, Turno, Paciente, Especialista_medico, Tipo_especialidad, Persona } = require("../db");
const router = Router();
...
asignandoAgenda? await crearTurno.setAgenda(asignandoAgenda): res.status(400).send({msg:"No se pudo encontrar la agenda indicada"});

const asignandoPaciente = await Paciente.findByPk(pacienteId);
asignandoPaciente? await crearTurno.setPaciente(asignandoPaciente): res.status(400).send({msg:"No se pudo encontrar el paciente"});
